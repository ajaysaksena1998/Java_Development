...
<body>
	...
	<table border="1" align="left">
		...
		<c:forEach items="${staff}" var="staff">
			<tr>
				<td width="10%">${staff.stuid }</td>
				<td width="10%">${staff.date}</td>
				<td width="10%">${staff.sapid}</td>
				<td width="10%">${staff.medicine}</td>
				<td width="10%"><a href="#" data-target="#staffmodal"
					data-toggle="modal" class="whi">Provide Medicines</a></td>
			</tr>
		</c:forEach>
	</table>
							...
							<form name="add_name" id="add_name" action="addmed" method="GET">
								...
								<div class="table-responsive">
									<table class="table table-bordered" id="dynamic_field">
										<tr>
											<td><select class="form-control" name="medicine[]">
											<option disabled="" value="" selected="">Select Medicines</option>
													<c:forEach items="${list}" var="list">
														<option value="${list.medicine}">${list.medicine}</option>
													</c:forEach>
											</select></td>
							...
							$('#add')
									.click(
											function() {
												...
												$('#dynamic_field')
														.append(
																'<tr id="row'+i+'"><td><select class="form-control" name="medicine[]"> <option disabled="" value="" selected="">Select Medicines</option> <c:forEach items="${list}" var="list"> <option value="${list.medicine}">${list.medicine}</option> </c:forEach> </select></td><td><input type="text" name="stock[]" placeholder="Enter Stock" class="form-control name_list" /></td><td><button type="button" name="remove" id="'+i+'" class="btn btn-danger btn_remove">X</button></td></tr>');
											});
